    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h2>Éditer le Compte Rendu</h2>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('rapports.update', $rapport->VIS_MATRICULE . '_' . $rapport->RAP_NUM) }}">
                    @csrf
                    @method('PATCH')
                    <div class="mb-3">
                        ...
                        <select class="form-control" id="PRA_NUM" name="PRA_NUM" required>
                            ...
                            <option value="{{ $praticien->PRA_NUM }}" {{ $rapport->PRA_NUM == $praticien->PRA_NUM ? 'selected' : '' }}>{{ $praticien->PRA_NOM }} {{ $praticien->PRA_PRENOM }}</option>
                            @endforeach
                        </select>
                        @error('PRA_NUM')
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="RAP_BILAN" name="RAP_BILAN" rows="4" required>{{ $rapport->RAP_BILAN }}</textarea>
                        @error('RAP_BILAN')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
